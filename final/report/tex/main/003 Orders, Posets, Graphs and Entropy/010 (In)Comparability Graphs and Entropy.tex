...
\stackrel{?}{\le} y$ question we might ask will give us exactly $1$ bit of
...
\begin{equation}
\operatorname{STAB}(G) := \operatorname{conv}\{\chi^S \in \R^V : S\text{ stable set in }G\}
\label{eq:stab}
\end{equation}

Formally, \ref{eq:stab} is the definition of the \emph{stable set polytope} of
an arbitrary graph $G$ with vertex set $V = \left\{{v_1, \cdots, v_n }\right\}$
