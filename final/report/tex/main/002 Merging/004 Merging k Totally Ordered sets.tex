...
We will use tools from Information Theory to prove that this algorithm is
optimal. The first tool we will be using is Shannon's entropy of a random
variable \(X\), \(H(X)\).
\begin{definition}
\begin{displaymath}
...
replaces two nodes of the pool with smallest weight by a new one which will
