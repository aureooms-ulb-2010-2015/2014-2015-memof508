...
\sum_{i=1}^{k} m_i$ is \BigOmega{\log \frac{m'}{m_1 \cdot m_2 \dots m_k}}.
\end{theorem}
...
\nb{Giving $\log \frac{m'}{m_1 \cdot m_2 \dots m_k}$ in the form of the
Stirling's approximation gives us $m' \log m' - \sum_{i=1}^{k} m_i \log m_i$
which clearly express the information contained in the sorted sequence $S'$ of
$m'$ elements minus the information we already have.}
...
approach. To each set \(s_i\) we assign the fake probability
