...
next section we will show how this algorithm can be used in the context of the
sorting \XY problem.
...
$j$, asking $b_1 \ask{\le} J(q)$ just means asking $b_1 \ask{\le} i_{J(q)}$
...
occurences'' of $b_1 = j$ in the set $\S$ hence
\begin{displaymath}
...
interval, $b_1 = J(q)$ for all $r$ in this interval, then $N_{b_1} \le
\frac{4\card{\S}}{2^r}$. Suppose that $b_1 = i$ and that the length of the
$\nth{i}$ interval, containing the $\nth{i}$ midpoint, is
$\frac{N_i}{\card{\S}} > \frac{4}{2^r}$, thus has radius $> \frac{2}{2^r}$. In
this interval we take two points $y_1 = \frac{m_1}{2^{r-1}}$ and $y_2 =
\frac{m_2}{2^{r-1}}$ such that $y_1$ lies on the left of the $\nth{i}$ midpoint
and $y_2$ on the right, hence $J(y_1) = i - 1$ and $J(y_2) = i$.
...
assume the current working interval is \((L,R]\). If during the search a
question \(b \ask{\le} J(q)\) gets a ``yes'' answer then we update the upper
bound and get a new working interval \((L,q]\), otherwise \(b > J(q)\) hence we
can update the lower bound and the working interval becomes \((q,R]\). In both
cases the lower bound remains excluded. At the beginning of the algorithm,
\(r - 1 = 0\), \(L=0\) and \(R=1\), hence \(c = 0\). Now for any working
interval $(\frac{c}{2^{r-1}}, \frac{c+1}{2^{r-1}}]$ the query point is \(q =
\frac{\sfrac{(c + (c + 1))}{2}}{2^{r-1}} = \frac{2c + 1}{2^r}\). Depending on the
outcome of the query \(b \ask{\le} J(q)\) the new working interval will be
either \((\frac{2c}{2^{r}}, \frac{2c+1}{2^{r}}]\) or \((\frac{2c+1}{2^{r}},
\frac{2c+2}{2^{r}}]\). \ref{lemma:xy:dt:qi} follows by induction.
\end{proof}

\ref{lemma:xy:dt:qi} gives us the guarantee that after $r-1$
