...
In this section we will expose an algorithm due to \citet*{fredman:1976}
...
n\) lattice. In the next section we will compute the lower bound for the
...
\begin{displaymath}
J(\frac{c}{2^{r-1}}) < i \le J(\frac{c+1}{2^{r-1}})
\end{displaymath}
\end{lemma}
...
bound and get a new query interval \((L,q]\), otherwise \(b > J(q)\) hence we
...
outcome of the query \(b \ask{\le} J(q)\) the new query interval will be
...
midpoint. Now, if this query interval contained more than just the \(\nth{i}\)
...
\begin{displaymath}
i-1 = J\left(\frac{c}{2^{r-1}}\right) < b_1 \le J\left(\frac{c+1}{2^{r-1}}\right) = i
\end{displaymath}
