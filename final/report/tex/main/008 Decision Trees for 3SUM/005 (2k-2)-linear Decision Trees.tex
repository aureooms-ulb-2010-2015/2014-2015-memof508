...
Step \textbf{1.} requires \BigO{n \log n} queries. Step \textbf{2} explicits
...
which, by \ref{theorem:fredman:1976}, would require \BigO{g^2} queries to sort.
In total, step \textbf{4.} would need \BigO{n^2} queries to be executed.
...
The rest of the algorithm is straightforward. The loop of step \textbf{5.} is
\BigO{n}. The loop of step \textbf{5.2.} is \BigO{\sfrac{n}{g}} since we
...
\BigO{n^2 g^{-1} \log g} for the second phase.
...
is \BigO{n \log n + gn}. We say that step \textbf{3.} is a comparison
