...
to make our algorithm achieve an upperbound of \BigO{n^3 \log^2 n \log m}
queries.
...
$x$, effectively computing cell $\cell$ containing $x$ \textbf{(\BigO{n^2 \log^2 n} queries)}
\item[2.] recursively build simplex $\simplex$ containing $x$ and inscribed to $\cell$
\textbf{(\BigO{n^3 \log^2 n} queries)}
