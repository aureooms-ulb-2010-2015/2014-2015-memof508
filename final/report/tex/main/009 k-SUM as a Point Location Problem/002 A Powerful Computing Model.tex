...
used for the detailed explanation of the same problem in \cite{burgisser:1997}.
...
effectively solving the $k$-SUM problem using \BigO{k n^3 \log^3 n} $n$-variate
linear queries.
...
\nb{Note that we always consider that $x$ is inside a cell that is not a cone.
...
each step is \BigO{\enetsize \log m}. However, we
...
is \BigO{n^3 \log^2 n \log m}. For our $k$-SUM problem, $m = \binom{n}{k}$,
